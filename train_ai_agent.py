...
import gym
from gym_a3c.pretrained_agent import get_pretrained_agent
import gym.wrappers
import numpy as np
...
def main():
    ...
    parser.add_argument('--env', type=str, default='Connect4-A3C-v0')
    ...
    def make_env(process_idx, test):
        ...
        #env.set_agent(get_pretrained_agent("/Users/DewiGould/Desktop/CodingProjects/Connect4/a3c_training/basically_random/10_finish/"))
    ...
    chainerrl.misc.draw_computational_graph(
        [agent.model.pi_and_v(np.array([np.array([[0. for _ in range(num_cols)] for _ in range(num_rows)], dtype=np.float32)]))[0]],
        os.path.join(args.outdir, 'model_pi'))
    chainerrl.misc.draw_computational_graph(
        [agent.model.pi_and_v(np.array([np.array([[0. for _ in range(num_cols)] for _ in range(num_rows)], dtype=np.float32)]))[1]],
